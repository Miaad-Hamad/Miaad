...
def get_assessment_forms(course_id,active_only=False):
    params={
        "select":"id,course_id,form_type,title,description,is_active",
        "course_id":f"eq.{int(course_id)}",
        "order":"id.asc"
...
def save_assessment_submission(course_id,form_row,participant_name,questions,answers):
    score=0.0
    ...
    for q in questions:
        ...
        if qtype=="multiple_choice":
            row["answer_text"]=str(raw) if raw is not None else None
            correct=str(q.get("correct_answer") or "")
            ok=(str(raw)==correct)
            pts=float(q.get("points") or 0) if ok else 0.0
            row["is_correct"]=ok
            ...
            row["answer_text"]=str(raw) if raw is not None else None
            row["numeric_value"]=float(raw) if raw is not None else None

        elif qtype=="yes_no":
            row["answer_text"]=str(raw) if raw is not None else None

        else:
            row["answer_text"]=str(raw).strip() if raw is not None else None
    ...
    saved=r.json()
    if not saved:
        raise RuntimeError("تعذر إنشاء سجل الاستجابة.")

    submission_id=int(saved[0]["id"])
    payload=[{"submission_id":submission_id,**row} for row in answer_rows]
...
def set_assessment_form_active(form_id,is_active):
    r=requests.patch(
        f"{URL}/rest/v1/assessment_forms",
        headers=H(SERVICE,"return=minimal"),
        params={"id":f"eq.{int(form_id)}"},
...
def assessment_links(course_data):
    course_id=int(course_data.get("id") or 0)
    slug=html.escape(str(course_data.get("slug") or ""))

    try:
        forms=get_assessment_forms(course_id,active_only=True)
    except requests.RequestException:
        st.warning("تعذر تحميل نماذج القياس والتقييم الآن.")
        return

    if not forms:
        st.info("لا يوجد نموذج قياس متاح حاليًا.")
        return
    ...
    labels={
        ...
        "satisfaction":("تقييم تجربة البرنامج","قياس رضا المتدربات عن محتوى البرنامج وتجربته التدريبية.")
    }
    order={"pre":1,"post":2,"satisfaction":3}
    forms=sorted(forms,key=lambda x:order.get(str(x.get("form_type") or ""),99))

    items=[]
    for f in forms:
        ft=str(f.get("form_type") or "")
        if ft not in labels:
            continue
        title,desc=labels[ft]
        items.append(
            f'<div class="course"><div><h3>{title}</h3><p class="small">{desc}</p></div>'
            f'<a class="btn primary" href="?page=assessment&slug={slug}&type={ft}">فتح النموذج ←</a></div>'
        )

    if items:
        B(f'<section class="sec"><div class="kick">القياس والتقييم</div>'
          f'<div class="fgrid" style="grid-template-columns:1fr">{"".join(items)}</div></section>')

def assessment_page():
    slug=st.query_params.get("slug","")
    form_type=st.query_params.get("type","pre")
    ...
    try:
        form_row=get_assessment_form(int(c["id"]),form_type)
    except requests.RequestException as e:
    ...
    try:
        questions=get_assessment_questions(int(form_row["id"]))
    except requests.RequestException as e:
    ...
    title=str(form_row.get("title") or "")
    desc=str(form_row.get("description") or "")
    B(f'<section class="ch"><div class="badge">FLOURISH</div><h1>{html.escape(title)}</h1><p class="copy">{html.escape(desc)}</p></section>')

    if not questions:
        st.info("لا توجد أسئلة في هذا النموذج.")
        foot()
        return

    version_key=f"assessment_version_{form_row['id']}"
    if version_key not in st.session_state:
        st.session_state[version_key]=0
    v=st.session_state[version_key]

    with st.form(f"assessment_form_{form_row['id']}_{v}",clear_on_submit=False):
        participant_name=st.text_input(
            "الاسم الكامل *",
            max_chars=100,
            key=f"assessment_name_{form_row['id']}_{v}"
        )
        ...
        for q in questions:
            ...
            label=qtext+(" *" if required else "")
            qtype=str(q.get("question_type") or "")
            widget_key=f"assessment_q_{qid}_{v}"

            if qtype=="multiple_choice":
                answers[qid]=st.radio(label,q.get("options") or [],index=None,key=widget_key)
            elif qtype=="scale_5":
                answers[qid]=st.radio(label,[1,2,3,4,5],horizontal=True,index=None,key=widget_key)
            elif qtype=="yes_no":
                answers[qid]=st.radio(label,["نعم","لا"],horizontal=True,index=None,key=widget_key)
            else:
                answers[qid]=st.text_area(label,max_chars=1000,key=widget_key)
    ...
    if submitted:
        if not participant_name or len(participant_name.strip())<2:
            ...
            if missing:
                st.error("أكملي جميع الأسئلة المطلوبة.")
            else:
                try:
                    ...
                    st.success("تم إرسال النموذج بنجاح، شكرًا لكِ.")
                except requests.RequestException as e:
...
def course_page():
    ...
    is_public_speaking=int(c.get("id") or 0)==2

    if is_public_speaking:
        assessment_links(c)
...
def admin():
    ...
    course_tab,assessment_tab,feedback_tab=st.tabs(["إدارة الدورات","القياس والتقييم","التقييمات"])
    ...
    with assessment_tab:
        st.markdown("### إتاحة نماذج القياس والتقييم")
        st.caption("هذا التحكم مستقل تمامًا عن خيار «إظهار الدورة في الموقع».")

        try:
            assessment_courses=admin_courses()
        except requests.RequestException as e:
            st.error(f"تعذر تحميل الدورات: {e}")
            assessment_courses=[]

        if not assessment_courses:
            st.info("لا توجد دورات.")
        else:
            ids=[int(x["id"]) for x in assessment_courses]
            cmap={int(x["id"]):x for x in assessment_courses}

            selected_course_id=st.selectbox(
                "اختاري الدورة",
                ids,
                format_func=lambda cid: f"{cmap[cid].get('name_ar') or 'برنامج تدريبي'} · {cmap[cid].get('course_date') or ''}",
                key="assessment_admin_course"
            )

            try:
                forms=get_assessment_forms(selected_course_id,active_only=False)
            except requests.RequestException as e:
                st.error("تعذر تحميل نماذج القياس والتقييم.")
                if e.response is not None:
                    st.code(e.response.text)
                forms=[]

            if not forms:
                st.info("لا توجد نماذج قياس مرتبطة بهذه الدورة.")
            else:
                labels={
                    "pre":("القياس القبلي","قياس ما قبل البرنامج."),
                    "post":("القياس البعدي","قياس ما بعد البرنامج."),
                    "satisfaction":("تقييم تجربة البرنامج","تقييم تجربة المتدربات.")
                }
                order={"pre":1,"post":2,"satisfaction":3}
                forms=sorted(forms,key=lambda x:order.get(str(x.get("form_type") or ""),99))
                ...
                for f in forms:
                    fid=int(f["id"])
                    ft=str(f.get("form_type") or "")
                    title,desc=labels.get(ft,(str(f.get("title") or "نموذج"),""))
                    ...
                    with st.container(border=True):
                        ...
                        with c1:
                            ...
                            st.caption(desc)
                            if active:
                                st.success("متاح الآن للمتدربات")
                            else:
                                st.caption("غير متاح حاليًا")

                        with c2:
                            if st.button(
                                "إيقاف النموذج" if active else "إتاحة النموذج",
                                key=f"toggle_assessment_{fid}",
                                use_container_width=True
                            ):
                                try:
                                    set_assessment_form_active(fid,not active)
                                    st.rerun()
                                except requests.RequestException as e:
                                    st.error("تعذر تحديث حالة النموذج.")
                                    if e.response is not None:
                                        st.code(e.response.text)
                        ...
                        try:
                            submissions_response=requests.get(
                                ...
                                headers=H(SERVICE),
                                params={
                                    "select":"id,participant_name,score,max_score,percentage,submitted_at",
                                    "form_id":f"eq.{fid}",
                                    "order":"submitted_at.desc"
                                },
                                timeout=15
                            ...
                            form_submissions=submissions_response.json()
                        except requests.RequestException as e:
                            form_submissions=[]
                            st.error("تعذر تحميل إجابات هذا النموذج.")
                            if e.response is not None:
                                st.code(e.response.text)

                        with st.expander(f"إجابات النموذج ({len(form_submissions)})"):
                            if not form_submissions:
                                st.info("لا توجد إجابات حتى الآن.")
                            else:
                                try:
                                    question_rows=get_assessment_questions(fid)
                                except requests.RequestException as e:
                                    question_rows=[]
                                    st.error("تعذر تحميل أسئلة النموذج.")
                                    if e.response is not None:
                                ...
                                for sub in form_submissions:
                                    sid=int(sub["id"])
                                    participant=str(sub.get("participant_name") or "بدون اسم")

                                    try:
                                        submitted_at=datetime.fromisoformat(
                                            str(sub.get("submitted_at") or "").replace("Z","+00:00")
                                        ).strftime("%d.%m.%Y · %H:%M")
                                    except:
                                        submitted_at=str(sub.get("submitted_at") or "")
                                    ...
                                    with st.container(border=True):
                                        ...
                                        if sub.get("percentage") is not None:
                                            st.write(f"**النتيجة:** {float(sub['percentage']):.1f}%")
                                        ...
                                        try:
                                            answers_response=requests.get(
                                                ...
                                                headers=H(SERVICE),
                                                params={
                                                    "select":"question_id,answer_text,numeric_value,is_correct,points_earned",
                                            ...
                                            answer_rows=answers_response.json()
                                        except requests.RequestException as e:
                                            answer_rows=[]
                                            st.error("تعذر تحميل إجابات هذه المتدربة.")
                                            if e.response is not None:
                                                st.code(e.response.text)

                                        for ans in answer_rows:
                                            q=question_map.get(int(ans["question_id"]),{})
                                            qtext=str(q.get("question_text") or "السؤال")
                                            qtype=str(q.get("question_type") or "")

                                            if qtype=="scale_5" and ans.get("numeric_value") is not None:
                                                answer_value=f"{int(float(ans['numeric_value']))}/5"
                                            else:
                                                answer_value=str(ans.get("answer_text") or "—")
